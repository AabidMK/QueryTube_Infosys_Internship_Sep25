...
class VideoSearchEngine:
    ...
    def _format_results(self, results: Dict[str, Any], min_score: float = 0.2) -> List[Dict[str, Any]]:
        ...
        for i in range(len(distances_list)):
            ...
            if score >= min_score:
                ...
                # Extract default URL from channel_thumbnail
                thumb = meta.get("channel_thumbnail", "")
                try:
                    if isinstance(thumb, str) and thumb.startswith("{"):
                        thumb_dict = eval(thumb)
                        thumb = thumb_dict['default']['url']
                except:
                    pass
    ...
    def display_results(self, formatted_results: List[Dict[str, Any]]):
        ...
        for r in formatted_results:
            ...
            print(f"Channel Thumbnail: {r.get('channel_thumbnail', 'N/A')}")
